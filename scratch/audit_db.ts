...
async function auditDatabase() {
  ...
  console.log("ORGANIZATIONS:", orgs?.length ?? 0, "registros", orgErr ? `ERRO: ${orgErr.message}` : "OK");
  ...
  console.log("\nSTORES:", stores?.length ?? 0, "registros", storeErr ? `ERRO: ${storeErr.message}` : "OK");
  if (stores) stores.forEach(s => console.log("  -", s.id, s.name, s.slug));

  // 3. Verificar profiles
  const { data: profiles, error: profErr } = await supabase.from('profiles').select('*');
  console.log("\nPROFILES:", profiles?.length ?? 0, "registros", profErr ? `ERRO: ${profErr.message}` : "OK");
  if (profiles) profiles.forEach(p => console.log("  -", p.id, p.role, p.full_name));

  // 4. Verificar auth.users
  const { data: { users }, error: userErr } = await supabase.auth.admin.listUsers();
  console.log("\nAUTH.USERS:", users?.length ?? 0, "registros", userErr ? `ERRO: ${userErr.message}` : "OK");
  if (users) users.forEach(u => console.log("  -", u.id, u.email, u.created_at));

  // 5. Verificar tabelas existem
  const tables = ['products', 'product_variants', 'categories', 'collections',
    'carts', 'cart_items', 'orders', 'order_items', 'coupons',
    'gift_cards', 'cash_registers', 'cash_shifts', 'cash_entries',
    'stock_reservations', 'pages', 'page_versions', 'stories',
    'chat_threads', 'chat_messages', 'reviews', 'commissions',
    'seller_showcases'];
  ...
  console.log(trigErr ? `Existe (erro esperado ao chamar sem NEW): ${trigErr.message}` : "Resultado inesperado");
}
