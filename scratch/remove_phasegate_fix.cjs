...
for (const file of files) {
  ...
  content = content.replace(/import\s*\{\s*PhaseGate\s*\}\s*from\s*['"]@\/components\/admin\/phase-gate['"];?[\r\n]*/g, '');
...
if (fs.existsSync(domainTs)) {
  ...
  domainContent = domainContent.replace(/export type RouteRenderStatus = "structural" | "planned";\n?/g, '');
