...
export const APIRoute = createAPIFileRoute("/api/webhooks/pagarme")({
  POST: async ({ request }: { request: Request }) => {
    try {
      ...
      const pagarmeWebhookKey = process.env.PAGARME_WEBHOOK_KEY || "test_key";
      
      // 1. Validate signature for security (Prevent spoofing)
      // Pagar.me sends a signature like: sha1=hash
      if (signature) {
        const expectedHash = crypto.createHmac("sha1", pagarmeWebhookKey).update(rawBody).digest("hex");
        if (`sha1=${expectedHash}` !== signature && pagarmeWebhookKey !== "test_key") {
      ...
      if (webhookError && webhookError.code === "23505") { // Unique violation
        // Already processed
        return new Response(JSON.stringify({ received: true, ignored: "already_processed" }), { status: 200 });
      }
